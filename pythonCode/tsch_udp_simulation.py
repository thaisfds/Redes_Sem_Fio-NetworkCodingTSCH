...
class TSCHNode:
    ...
    def send_message(self, destination: int, data: str, direct: bool = False):
        with self.lock:
            ...
            # --- NOVO: Contadores de Mensagens Globais ---
            ...
            if is_direct_possible:
                ...
                if next_hop:
                    ...
                    # NOVO: Mensagem não enviada porque não encontrou rota
    ...
    def receive_message(self, message: Message):
        with self.lock:
            ...
            # NOVO: Cada vez que uma mensagem é recebida para encaminhamento, conta como um "reencaminhamento"
            # Isso pode ser um proxy para "retransmissão" no sentido de "esforço de rede"
            if message.destination != self.node_id:
                self.network.total_forwarded_messages += 1 # Conta reencaminhamentos
            ...
            if message.destination == self.node_id:
                ...
                if message.hop_count < 5:
                    ...
                    if next_hop and next_hop != message.source:
                        ...
                        # NOVO: Mensagem descartada por não ter para onde encaminhar
                    ...
                    # NOVO: Mensagem descartada por limite de hops
...
class TSCHNetwork:
    def __init__(self):
        ...
        self.network_coding_enabled: Optional[int] = 0 # Define como 0 por padrão, já que não será lido explicitamente
        self.nc_nodes: List[int] = [] # Para armazenar os nós de Network Coding
        self.nc_senders: List[int] = [] # Para armazenar os nós que enviam para NC

        # --- NOVOS CONTADORES DE MÉTRICAS ---
        self.total_messages_initiated = 0 # Mensagens que os nós tentaram enviar (originadas)
        self.total_forwarded_messages = 0 # Mensagens que foram reencaminhadas (hops intermediários)
        self.messages_not_sent_no_route = 0 # Mensagens que não encontraram rota inicial
        self.messages_dropped_hop_limit = 0 # Mensagens que foram descartadas por atingir limite de hops
        self.messages_dropped_no_forward_path = 0 # Mensagens descartadas porque não havia mais para onde encaminhar
        # ------------------------------------
    ...
    def parse_config_file(self, filename: str):
        ...
        Sempre pula as duas primeiras linhas, que são reservadas para informações de Network Coding.
        """
        try:
            with open(filename, 'r') as f:
                lines = [line.strip() for line in f if line.strip() and not line.startswith('#')]

                # --- MODIFICAÇÃO AQUI: Sempre pula as 2 primeiras linhas ---
                if len(lines) < 2:
                    print(f"ERROR: Configuration file '{filename}' has less than 2 lines. It must contain NC nodes and NC senders before topology data.")
                    sys.exit(1)

                # Lê e armazena (opcionalmente) as informações das duas primeiras linhas
                # Mesmo que o programa "ignore" para o fluxo principal, você pode tê-las aqui para futura implementação de NC
                self.nc_nodes = [int(x) for x in lines[0].split()]
                self.nc_senders = [int(x) for x in lines[1].split()]

                lines = lines[2:] # Remove as 2 primeiras linhas
                # --------------------------------------------------------
    ...
    def run_simulation(self, output_base_path: str, output_file_base_name: str, results_csv_file_obj):
        ...
        # Removendo a linha sobre "Network Coding: Ativado/Desativado" na saída de console, já que não temos um marcador.
        # Se Network Coding fosse implementado, esta linha seria gerada com base em alguma lógica interna ou de configuração.
        ...
        if results_csv_file_obj:
            ...
            row_data = [
                ...
                total_paths_found, # Mensagens entregues com sucesso
                slotframe_length,
                total_transmissions,
                num_channels_used,
                self.network_coding_enabled, # Será 0, pois não há marcador explícito
                # --- NOVO: Adiciona as novas métricas ao CSV ---
    ...
    try:
        ...
        if not file_exists:
            csv.writer(results_csv_file).writerow([
                ...
                "Numero_de_Mensagens_Config", # Total de mensagens que o arquivo pediu para enviar
                "Numero_de_Caminhos_Completos", # Mensagens que chegaram ao destino final
                "Tamanho_do_Slotframe",
                "Numero_de_Transmissoes_Totais", # Total de hops/envios de rádio (sucesso ou falha)
                "Numero_de_Canais_Usados",
                "Network_Coding", # Campo para o valor de N ou 0
                # --- NOVOS CAMPOS PARA O CSV ---
                "Mensagens_Iniciadas",          # Total de chamadas a send_message (originadas)
                "Mensagens_Nao_Enviadas_Sem_Rota", # Falhas logo no primeiro hop (get_next_hop retornou None)
                "Mensagens_Descartadas_Hop_Limit", # Falhas no roteamento (atingiu limite de hops)
                "Mensagens_Descartadas_Sem_Prox_Caminho", # Falhas no roteamento (next_hop mas não vizinho)
                "Total_Retransmissoes_Logicas" # Total de hops intermediários bem-sucedidos
                # ------------------------------
            ])
